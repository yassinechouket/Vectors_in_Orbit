...
class ResponseFormatter:
    ...
    def _format_recommendation(
        ...
        # Format product (prices are in TND)
        ui_product = UIProduct(
            id=product.id,
            name=product.name,
            price=product.price,
            formatted_price=f"{product.price:,.2f} TND",
    ...
    def _format_query_understanding(self, intent: ParsedIntent) -> Dict[str, Any]:
        ...
        # Budget is in USD (user input), will be converted to INR for search
        return {
            "category": intent.category or "All categories",
            "budget": f"${intent.max_price:,.0f}" if intent.max_price else "No limit",
