...
class SearchFilters:
    ...
    max_price: Optional[float] = None
    min_price: Optional[float] = None
    categories: List[str] = field(default_factory=list)  # Multiple categories for OR search
    ...
    def to_qdrant_filter(self) -> Dict[str, Any]:
        ...
        # Support multiple categories with OR condition
        if self.categories:
            if len(self.categories) == 1:
                must.append({
                    "key": "category",
                    "match": {"value": self.categories[0].lower()}
                })
            else:
                # Use "should" for OR between multiple categories
                category_conditions = []
                for cat in self.categories:
                    category_conditions.append({
                        "key": "category",
                        "match": {"value": cat.lower()}
                    })
                must.append({
